...
if(isset($_POST['submit'])){
  ...
  else
  {
    ...
    $fetch=$login->fetch(PDO::FETCH_ASSOC);//ASSOC=associative array

    if($login->rowCount() > 0){//checking that in db table exists email or not if rowcount is 0 then there is no email
      //echo "<script>alert('email is fine');</script>";

      //check for the password with password verify
      if(password_verify($password, $fetch['mypassword'])){
        //echo "<script>alert('password is fine');</script>";//to send msg that password is fine
